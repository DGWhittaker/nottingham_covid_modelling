...
def plot_mcmc_series_posterior_samples():
    parser = get_parser()
    ...
    for i in range(nsamples):
        ...
        S_vec.append(S)
        if not label_added:
            ax1.plot(t[:-p.numeric_max_age], D[:-p.numeric_max_age], color='dodgerblue', alpha=0.01, label='MCMC posterior samples', zorder=-1)
            if args.show_Itot:
                ax2.plot(t[:-p.numeric_max_age], Itot[:-p.numeric_max_age], color='dodgerblue', label='Daily active', alpha=0.01)
            ax2.plot(t[:-p.numeric_max_age], I[0, :-p.numeric_max_age], color='orange', label='New', alpha=0.01)
            label_added = True
        else:
            ax1.plot(t[:-p.numeric_max_age], D[:-p.numeric_max_age], color='dodgerblue', alpha=0.01, zorder=-1)
            if args.show_Itot:
                ax2.plot(t[:-p.numeric_max_age], Itot[:-p.numeric_max_age], color='dodgerblue', alpha=0.01)
            ax2.plot(t[:-p.numeric_max_age], I[0, :-p.numeric_max_age], color='orange', alpha=0.01)
    ...
    if args.show_Itot:
        ax2.plot(t[:-p.numeric_max_age], Itot[:-p.numeric_max_age], color='blue', label='Daily active', alpha=0.01)
    ...
    l_alpha = len(p.alpha)
    for i in range(nsamples):
        paras = chains[args.chain - 1][values[i]]
        p_dict = dict(zip(LL.parameter_labels, paras))
        ...
        if not label_added:
            axins.plot(p.alpha[:-(p.numeric_max_age + p.extra_days_to_simulate)], color='dodgerblue', alpha=0.01, label='MCMC posterior samples')
            ax3.plot(R_eff, color='red', alpha=0.01, label='MCMC posterior samples')
            label_added = True
        else:
            axins.plot(p.alpha[:-(p.numeric_max_age + p.extra_days_to_simulate)], color='dodgerblue', alpha=0.01)
            ax3.plot(R_eff, color='red', alpha=0.01)
