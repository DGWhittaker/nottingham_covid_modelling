...
FITING_MAPPING = {'default': ['rho', 'Iinit1'], 'full': ['rho', 'Iinit1', 'theta', 'xi']}
...
def run_optimise():
    ...
    if Fitparams == 'full':
        ...
        parameters_to_optimise_SIR = ['rho', 'Iinit1', 'theta']
        parameters_to_optimise_SIRDeltaD = ['rho', 'Iinit1', 'theta', 'DeltaD']
    else:
        parameters_to_optimise_SIR = ['rho', 'Iinit1']
        parameters_to_optimise_SINR = FITING_MAPPING[Fitparams]
        parameters_to_optimise_SIRDeltaD = ['rho', 'Iinit1']
    if FitStep:
    ...
    filename_SIR = os.path.join(folder_path, get_file_name_suffix(p, 'syntheticSItRD-'+Noise_flag, 'SIR-'+ Fitparams + '-fitStep-' + str(FitStep) + '-maxtime-' + str(maxtime_fit), parameters_to_optimise_SIR) + travel_label)
    filename_SIRDeltaD = os.path.join(folder_path, get_file_name_suffix(p, 'syntheticSItRD-'+Noise_flag, 'SIRDeltaD-'+ Fitparams + '-fitStep-' + str(FitStep) + '-maxtime-' + str(maxtime_fit), parameters_to_optimise_SIR) + travel_label)
    filename_SINR = os.path.join(folder_path, get_file_name_suffix(p, 'syntheticSItRD-'+Noise_flag, 'SINR-'+ Fitparams +  '-fitStep-' + str(FitStep) + '-maxtime-' + str(maxtime_fit), parameters_to_optimise_SINR) + travel_label)
    filename_plot = os.path.join(folder_path, 'SyntheticSItD-NBnoise-fit-' + Noise_flag + Fitparams + '-fitStep-' + str(FitStep) + '-fitSItR-' + str(FitAge) + '-maxtime-' + str(maxtime_fit) + travel_label)
    ...
    data_Dnoise = data_Dnoise[:p.maxtime+1]
    ...
    data_D = data_D[p.day_1st_death_after_150220:]
    print(len(data_D))
    ...
    def minimise(params, p, parameters_dictionary, travel_data, data_D):
    ...
    def minimise_SIR(params, p, parameters_dictionary, data_D, travel_data):
        p_dict = dict(zip(parameters_dictionary, params))
...
#        return -f
    ...
    if Fitparams == 'full':
        bounds_SIR = [[rho_SIR_lower, Iinit1_lower, theta_lower], [ rho_SIR_upper, Iinit1_upper, theta_upper]]
        bounds_SIRDeltaD = [[rho_SIR_lower, Iinit1_lower, theta_lower, DeltaD_lower], [ rho_SIR_upper, Iinit1_upper, theta_upper, DeltaD_upper]]
        bounds_SINR = [[rho_SIR_lower, Iinit1_lower, theta_lower, xi_lower], [ rho_SIR_upper, Iinit1_upper, theta_upper, xi_upper]]
        stds_SIR = [1e-2, 100, 1e-2]
        stds_SIRDeltaD = [1e-2, 100, 1e-2, 10]
        stds_SINR = [1e-2, 100, 1e-2, 1e-2]
    else:
        bounds_SIR = [[ rho_SIR_lower, Iinit1_lower], [ rho_SIR_upper, Iinit1_upper]]
        bounds_SIRDeltaD = [[ rho_SIR_lower, Iinit1_lower], [ rho_SIR_upper, Iinit1_upper]]
        bounds_SINR = [[ rho_SIR_lower, Iinit1_lower], [ rho_SIR_upper, Iinit1_upper]]
        stds_SIR = [1e-2, 100]
        stds_SIRDeltaD = [1e-2, 100]
        stds_SINR = [1e-2, 100]
    ...
    for i in range(repeats):
        ...
        print(x0_SIR)
        es = cma.fmin(minimise_SIR, x0_SIR, sigma0=1, args=(p, parameters_to_optimise_SIR, data_D, travel_data), options = optsSIR)
    ...
    for i in range(repeats):
        ...
        print(x0_SIRDeltaD)
        es = cma.fmin(minimise_SIR, x0_SIRDeltaD, sigma0=1, args=(p, parameters_to_optimise_SIRDeltaD, data_D, travel_data), options = optsSIRDeltaD)
    ...
    for i in range(repeats):
        ...
        print(x0_SINR)
        es = cma.fmin(minimise_SINR, x0_SINR, sigma0=1, args=(p, parameters_to_optimise_SINR, data_D, travel_data), options=optsSINR)
    ...
    if FitAge:
        ...
        toy_values = [3, 1000]
        filename_AGE = os.path.join(folder_path, get_file_name_suffix(p, 'syntheticSItRD', Noise_flag+'-maxtime-' + str(maxtime_fit), parameters_to_optimise))
        
        bounds_SItR = [[ rho_lower, Iinit1_lower], [ rho_upper, Iinit1_upper]]
        stds_SItR = [1e-1, 100]
        if FitStep:
        ...
        for i in range(min(repeats, 2)):
            ...
            print(x0)
            es = cma.fmin(minimise, x0, sigma0=1, args=(p, parameters_to_optimise, travel_data, data_D), options=opts)
