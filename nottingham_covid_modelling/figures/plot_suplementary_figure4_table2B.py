...
folder_fits = os.path.join(MODULE_DIR, 'out-mcmc-SubTable2B')
...
table_2_MAPS = np.zeros([5,9])
table_2_derived = np.zeros([5,5])
...
for i, model_i in enumerate(Model_name): 
    ...
    if model_i == 'SItD':
        ...
        filename = get_file_name_suffix_anymodel(p, 'ONS-UK', '','', model_i , params_opt_names[model_i])
